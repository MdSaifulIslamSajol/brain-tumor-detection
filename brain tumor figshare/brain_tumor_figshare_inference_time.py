...
sys.stdout = open(f"./results/console_output_lr{learning_rate}_bs{batchsize}_inferenc_etime.txt", "w")
...
dataset_path = "/data/saiful/pilot projects/Brain Tumor Figshare/Fig Share/"
...
# List all PNG image files in the dataset path
image_extensions = ['.png']  # Only checking for PNG files
all_image_paths = [os.path.join(root, file)
                   for root, _, files in os.walk(dataset_path)
                   for file in files
                   if os.path.splitext(file)[1].lower() in image_extensions]

# Check if all image paths are correctly formatted
...
complete_dataset = CustomDataset(all_image_paths, transform=data_transform)
# Split the complete dataset into 80% training and 20% test sets
train_size = int(0.8 * len(complete_dataset))
test_size = len(complete_dataset) - train_size
trainset, testset = random_split(complete_dataset, [train_size, test_size])
