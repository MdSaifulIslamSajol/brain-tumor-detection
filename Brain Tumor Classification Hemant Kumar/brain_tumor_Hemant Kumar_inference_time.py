...
# https://www.kaggle.com/datasets/jarvisgroot/brain-tumor-classification-mri-images/data
# torch
import torch
...
sys.stdout = open(f"./results/console_output_lr{learning_rate}_bs{batchsize}_Hemant_Kumar_inference_time.txt", "w")
...
# Load data
training_data = pickle.load(open("/data/saiful/pilot projects/Brain Tumor Classification Hemant Kumar/brain_tumor_mri/new_dataset/training_data.pickle", 'rb'))
...
class_mapping = {1: 'Meningioma', 2: 'Glioma', 3: 'Pitutary'}
n_classes = len(classes)

# Separate images and labels
x_raw = [item[0] for item in training_data]
y_raw = [item[1] for item in training_data]

# Print unique values in y_raw
unique_labels = set(y_raw)
# Map class names to numeric labels
y_raw_mapped = [class_mapping[label] for label in y_raw]


print(f'Total number of images: {total_data_len}')
plt.imshow(x_raw[0])
plt.title(f'Class: {y_raw_mapped[0]}')  # Displaying class name directly
plt.show()

# Count unique values using NumPy
unique, counts = np.unique(y_raw, return_counts=True)
unique_counts = dict(zip(unique, counts))  #  {1: 1416, 2: 2852, 3: 1860}

print("Unique value counts in y_raw (using NumPy):")
print(unique_counts.items())
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = self.labels[index] - 1  # Adjust label to start from 0
...
# Assuming train_transform and test_transform are defined as given in your original code
train_transform = transforms.Compose([
    transforms.RandomHorizontalFlip(),
    transforms.RandomVerticalFlip(),
    transforms.RandomRotation(10),
    transforms.Resize((224, 224)),  # Example image size, modify as needed
    transforms.ToTensor(),
...
# Split data into train and test sets (Assuming some split logic or lists are defined)
# Here, for demonstration, let's assume x_raw and y_raw are split into train and test sets
train_indices = int(0.8 * len(x_raw))
train_images, test_images = x_raw[:train_indices], x_raw[train_indices:]
train_labels, test_labels = y_raw[:train_indices], y_raw[train_indices:]

trainset = CustomDataset(train_images, train_labels, transform=train_transform)
testset = CustomDataset(test_images, test_labels, transform=test_transform)

# Example to test dataset loading
trainloader = torch.utils.data.DataLoader(trainset, batch_size=32, shuffle=True, num_workers=4)
testloader = torch.utils.data.DataLoader(testset, batch_size=32, shuffle=False, num_workers=4)

# Print some examples from the train loader to verify
...
# Hemant kumar Dataset Normalization
# Training set mean: tensor([-1.4455, -1.3483, -1.1201])
# Training set std: tensor([0.6913, 0.7067, 0.7036])
# Test set mean: tensor([-1.4403, -1.3429, -1.1148])
# Test set std: tensor([0.6919, 0.7073, 0.7042])
...
def train_test_plot(method_name="_"):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for j, (inputs, labels) in enumerate(testloader):
                ...
                ret, predictions_batch = torch.max(outputs.data, 1)
                correct_counts = predictions_batch.eq(labels.data.view_as(predictions_batch))
                acc = torch.mean(correct_counts.type(torch.FloatTensor))
                valid_acc += acc.item() * inputs.size(0)
                true_labels.extend(labels.cpu().numpy())
                predictions.extend(predictions_batch.cpu().numpy())

            inference_time_per_image = total_inference_time / total_images
            print("len(total_test_images) :", total_images)
            print(f'Inference time per image for {method_name}: {inference_time_per_image:.6f} seconds')
    ...
    plt.ylim(0, 100)
    plt.tight_layout()
    plt.savefig(f"./results/{method_name}_lr{learning_rate}_bs{batchsize}_document_dataset_accuracy_curve_IEEE.png", dpi=300)
    plt.show()
    ...
    plt.savefig(f"./results/{method_name}_lr{learning_rate}_bs{batchsize}_confusion_matrix.png", dpi=300)
    plt.show()
